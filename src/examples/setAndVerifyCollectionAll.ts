...
async function setAllAndSaveErrors(
  mints: PublicKey[],
  no: NFTsOperator,
  collectionAddress: PublicKey,
  updateAuthority: Keypair,
): Promise<SignatureWithMint[]> {
  const { results, errors } = await PromisePool.for(mints)
    .withConcurrency(10)
    .onTaskFinished((mint: PublicKey, pool: any) => {
      const count = pool.processedCount();
      count % 100 === 0 && console.log(count, "/", mints.length);
    })
    .process(async (mint: PublicKey) => {
      const signature = await no.setAndVerifyCollection(
        mint,
        collectionAddress,
        updateAuthority,
      );
  ...
  const resultsWithStringifiedMints = results.map((r) =>
    Object.assign(r, { mint: r.mint.toString() }),
  );
  file.writeJson({
...
async function verifyAllTxs(
  ...
  signatureWithMints: SignatureWithMint[][],
): Promise<SignatureMintError[]> {
  console.log("Verifying txs");
  const { results, errors } = await PromisePool.for(signatureWithMints)
    ...
    .process(async (signatureWithMint: SignatureWithMint[]) => {
      const statuses = await connection.getSignatureStatuses(
        signatureWithMint.map((s) => s.signature),
      ...
      statuses.value.forEach((s, i) => {
        if (
          s?.err &&
          s?.err.InstructionError &&
          s?.err.InstructionError[1]?.Custom === 114
        )
          return;
        if (!s || s?.err)
          r.push(Object.assign({ err: s?.err }, signatureWithMint[i]));
      });
      return r;
    });
  errors.length &&
...
async function run() {
  try {
    ...
    let mints = file
      .loadJson("deps/mint_addresses.json")
      .map((a: string) => new PublicKey(a));
    let tryAgain = true;
    while (tryAgain) {
      tryAgain = false;
      const results = await setAllAndSaveErrors(
        mints,
        no,
        collectionAddress,
        updateAuthority,
      ...
      const fails = await verifyAllTxs(
        connection,
        sliceIntoChunks(results, 255),
      );

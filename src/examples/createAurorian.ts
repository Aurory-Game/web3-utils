import { CreatorInput, keypairIdentity, Metaplex } from "@metaplex-foundation/js";
...
async function run() {
  try {
    ...
    const aurorianCollection = new PublicKey('7BQdHnBKERaCYCnwLbbSoYHQZxcq7zLenYERDp94o18z');

    const uri = 'https://assets.cdn.aurory.io/aurorians/10000.json';
    const aurorianData: any = await loadJsonFromUri(uri);
    const r = await metaplex.nfts().create({
      isCollection: false,
      ...
      creators: aurorianData.properties.creators.map(({address, share}: {address: string, share: number}) => {
